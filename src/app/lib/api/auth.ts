...
export const login = async (credentials: LoginCredentials): Promise<AuthResult> => {
  try {
    ...
    console.log("Raw login response data:", JSON.stringify(data));
    
    if (data.status === 'success' && data.data.token) {
      console.log("Login successful, checking for password_change_required flag:", {
        has_flag: typeof data.data.password_change_required !== 'undefined',
  ...
  try {
    ...
    if (apiError.response?.status === 422) {
      ...
      if (apiError.response.data?.errors) {
        ...
        // Set a more specific error message based on the validation errors
        ...
        if (requestData?.email) {
          ...
          errorMessage = 'Please check your input and try again.';
          errorDetails = {
            general: 'The provided information is invalid. Please check all fields and try again.'
          };
        }
      }
